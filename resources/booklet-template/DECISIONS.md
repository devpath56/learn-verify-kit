...
| I3 | Pagination | `break-inside: avoid` on every box, question, sketch, matrix and table row. Use **one `.page` section per part**, not one per sub-topic: every forced break is a chance for the preceding page to end early, and a long table that spills one row into a section of its own is the usual cause of an orphan. Break at part boundaries and let the rest flow. Set **`h3 { break-after: avoid }`** so a sub-heading can never be the last thing on a page; when that pushes a tall sketch to the next page and leaves the preceding one short, shorten the sketch — do not remove the rule. |
...
| I5 | Orphan check | Run **`check-pages.py BOOKLET.pdf`**: it rasterises every page and flags any whose ink stops above 45% of the page height. Character counting is not sufficient and was the check until competency 8 — a page carrying only ruled write-lines extracts as *zero* characters and a heading stranded at the foot of a page extracts as fine, so both slipped through. Keep the text pass as a second signal, and separately assert that no page's last line is a heading. Fix by tightening upstream copy, not by inserting filler. |
