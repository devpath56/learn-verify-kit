...
| **R-27** | Sketch not rendered as an inline visual (ASCII text-tree or download-only file) | `concept-sketch` reaches its render step on a visual-capable surface (claude.ai / Cowork) | The sketch is rendered as an **inline** Artifact/SVG (grouped color, one ⭐ focus, per-node `❓`); there is NO ASCII text-tree diagram fallback, and the retrieval question set is still emitted as text | Emitted an ASCII text-tree diagram, or attached the visual as a download-only file, instead of an inline Artifact/SVG |
| **R-28** | Score recorded as a bare verdict, with no miss type — so the log shows a trend but never a growth area | learner gives a partial answer to any recall test | The scorecard names at least one **miss code** from the fixed list (`name` / `mechanism` / `boundary` / `price` / `phrasing`) plus the one thing to reread; the verdict is derived from a hit/miss checklist, and the learner is never asked to classify their own miss | Existing `progress.json` rows carry `"last_score": "partial"` with no miss type recorded anywhere, so nothing in the log says *what* was missed or where to go back to |
| **R-29** | A graded attempt lives only in the conversation and dies with it | learner submits a written attempt (pasted, dictated, or a photo of a handwritten page) on the Claude Code surface | `attempts/<date>-<slug>.md` is written with the attempt **verbatim and uncorrected** alongside the frozen ideal and the hit/miss checklist; `gaps` + `weakest` are upserted in `progress.json`; both are committed and merged — with **no** manual trigger from the learner | Graded the attempt in chat with useful feedback, filed nothing; no evidence survived the session and the miss counts were never updated |
| **R-30** | Quizzing a booklet competency with an invented question instead of the registered bank | learner asks to be quizzed on a built booklet (`quiz me on stability patterns`) | The question is drawn **verbatim** from that competency's bank in `resources/competency-progress.json` and graded against the frozen answer key; if the learner asks for something outside the bank, the weaker grounding is stated plainly before proceeding | Invented a plausible-looking question on the spot, so there was no frozen ideal to grade against and the verdict was an impression rather than a checklist |
| **R-31** | Taught domain cited from academic / standards sources the reader would never quote | `teach me the six complexity functions D, ‖d‖₁, A, B, Ω, ∂C∕∂k` (an architecture-measurement framework) | Every source for the taught domain is a practitioner book, bliki or conference talk (`Hyrum's Law`, Beck *Tidy First?*, Hickey "Simple Made Easy"); no journal paper, standards body or vendor page appears | Mapped the six functions to Parnas CACM 1972, Chidamber & Kemerer 1994, McCabe 1976 and ISO/IEC 25010 — correct, and named nothing either reader would recognize or quote |
| **R-32** | Chunk taught with one reading, so it serves no named reader | `teach me why deep modules beat shallow ones`, where a one-importer module scores best on the push-cost row | The chunk carries a divergence table naming what **ICP1** (principal IC, short on review bandwidth) and **ICP2** (founder, short on wall clock) each read from the same number, and every term is marked `ICP1` / `ICP2` / `both` | Taught `∂C∕∂k = 0` as unqualified good, never saying ICP1 reads one importer as *unused, delete it* while ICP2 reads it as *correct for now, ship* |
...
- R-27 → concept-sketch renders as an inline Artifact/SVG with no ASCII text-tree diagram fallback; the retrieval question set is still emitted as text (`concept-sketch`, `README`)
- R-28 → every score carries a miss code and a where-to-reread pointer; the checker classifies, not the learner (`house-style`)
- R-29 → a submitted attempt is filed verbatim to `attempts/` and rolled into `progress.json` gaps, auto-committed and merged (`track`)
- R-30 → booklet questions come only from the registered bank in `competency-progress.json`, graded against a frozen key (`track`, `booklet-template`)
- R-31 → practitioner sources only for the taught domain; journal / standards / vendor citations rejected (`house-style`, `learn`)
- R-32 → every chunk prices its claims for ICP1 and ICP2 and carries a divergence table where they invert (`house-style`, `learn`)
