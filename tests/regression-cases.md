...
| **R-28** | Columnar data (2+ items sharing the same fields) rendered as a repeated per-item bullet stack instead of a table | `learn state matrix` (chunk 1 introduces 3 terms — state, state matrix, treatment — each with the same name+plain-definition+example+boundary fields) | The shared-schema term block is rendered as ONE table whose columns are name / definition / example / boundary; no per-item bullet stack that repeats "Term — plain: … Example: … Boundary: …" | Emitted the 3 terms as a stack of bullets, each repeating "plain: … Example: … Boundary: …", when every item shared the same 4 fields and was plainly a 4-column table |
...
- R-28 → shared-schema columnar data (esp. the name+definition+example+boundary term block) must be one table, not a repeated per-item bullet stack (`house-style`)
