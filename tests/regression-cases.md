...
| **R-27** | Text-only sketch on a surface that can render inline visuals | `concept-sketch` reaches its render step on a visual-capable surface (claude.ai / Cowork) — e.g. any consolidation there | On a visual-capable surface the sketch is rendered as an **inline** Artifact/SVG (grouped color, one ⭐ focus, per-node `❓`), carrying the same chunks/focus/questions; the text tree is used only as the terminal fallback | Defaulted to a monospace text tree on claude.ai, or attached the visual as a download-only file, when an inline Artifact/SVG was possible |
...
- R-27 → on visual-capable surfaces the sketch renders as an inline Artifact/SVG, not a text tree or a download-only file; text tree is the terminal fallback (`concept-sketch`)
