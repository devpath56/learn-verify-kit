...
- **Columnar data MUST be a table, not repeated bullets.** When **2+ items share the same fields** — most importantly the **name + plain-definition + example + boundary** block introduced for every term, but also any state×treatment / option×tradeoff / row-per-item set — render it as **one table** whose columns are those shared fields, not as a per-item stack of bullets that silently repeats the same structure. If you catch yourself writing "Term — plain: … Example: … Boundary: …" for a second item, stop and tabulate. Bullets are for prose points that don't share a schema; a shared schema across items *is* a table.
...
- [ ] Do 2+ items share the same fields (esp. the name+definition+example+boundary term block)? -> they are rendered as ONE table with those fields as columns, NOT a repeated per-item bullet stack
